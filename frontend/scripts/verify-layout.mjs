...
const assertions = [
  ...
  ["sidebar emits category select", files.layout.includes("defineEmits") && files.layout.includes("select-category")],
  ["sidebar receives active category", files.layout.includes("activeCategory")],
  ...
  ["home fetches banners from API", files.home.includes("fetch(") && files.home.includes("/home/banners/")],
  ["home fetches recommendations from API", files.home.includes("fetch(") && files.home.includes("/home/recommendations/")],
  ["home maps new video fields", files.home.includes("badge_text") && files.home.includes("status_text") && files.home.includes("subtitle") && files.home.includes("tags")],
  ["home maps video file url", files.home.includes("video_file_url") && files.home.includes("videoUrl")],
  ["home normalizes media url", files.home.includes("normalizeMediaUrl") && files.home.includes("http://127.0.0.1:8000")],
  ["home normalizes home category", files.home.includes("normalizeCategory")],
  ["home opens video player", files.home.includes("openPlayer") && files.home.includes("selectedVideo")],
  ["home renders video modal", files.home.includes("video-player-modal") && files.home.includes("<video")],
  ["home closes video player", files.home.includes("closePlayer")],
  ["home renders recommendation badges", files.home.includes("recommend-badge") && files.home.includes("badgeClass(item)")],
  ["home renders status text", files.home.includes("recommend-status") && files.home.includes("item.statusText")],
  ["home has loading state", files.home.includes("isLoading") && files.home.includes("banner-loading")],
  ["home tracks current category", files.home.includes("currentCategory")],
  ["home passes active category to layout", files.home.includes(":active-category=\"currentCategory\"")],
  ["home handles category selection", files.home.includes("@select-category=\"selectCategory\"")],
  ["home fetches category query", files.home.includes("category=${encodeURIComponent(normalizedCategory)}")],
  ["home has empty state", files.home.includes("recommend-empty") && files.home.includes("暂无相关剧集")],
  ["home removed hardcoded banner array", !files.home.includes("const banners = [")],
  ["home removed hardcoded recommendation array", !files.home.includes("const recommendations = [")],
  ...
  ["badge styles include vip and exclusive", files.css.includes(".recommend-badge.is-vip") && files.css.includes(".recommend-badge.is-exclusive")],
